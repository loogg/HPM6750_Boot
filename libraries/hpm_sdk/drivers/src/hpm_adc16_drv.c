...
#ifndef ADC16_CALI_REG_LEN
#define ADC16_CALI_REG_LEN  (34U)
#endif
...
static hpm_stat_t adc16_do_calibration(ADC16_Type *ptr)
{
    ...
    uint32_t adc16_params[ADC16_CALI_REG_LEN];
    int32_t  param01;
    uint32_t param02;
    uint64_t param64;
    uint32_t param32;
    ...
    for(i = 0; i < ADC16_CALI_REG_LEN; i++) {
        adc16_params[i] = 0;
    }
    ...
    for (i = 1; i < ADC16_CALI_REG_LEN - 2; i++) {
        adc16_params[i] = adc16_params[ADC16_ADC16_PARAMS_ADC16_PARA32] + adc16_params[i] - adc16_params[ADC16_ADC16_PARAMS_ADC16_PARA33] + adc16_params[i - 1];
    }
...
hpm_stat_t adc16_init(ADC16_Type *ptr, adc16_config_t *config)
{
    ...
    if (config->ch > ADC16_BUS_RESULT_CHN7) {
...
hpm_stat_t adc16_set_period_config(ADC16_Type *ptr, adc16_prd_config_t *config)
{
    ...
    prd_freq = config->clk_src_in_hz / (config->prescale + 1);
    prd_reload_value = config->period_ms / 1000 * prd_freq;
...
hpm_stat_t adc16_set_preempt_config(ADC16_Type *ptr, adc16_trig_config_t *config)
{
    ...
    for (int i = 0; i < config->trig_len; i++) {
        if (config->adc_ch[i] > ADC16_SAMPLE_CFG_CHN7) {
            return status_invalid_argument;
        }

        temp |= config->inten[i] << (ADC16_CONFIG_INTEN0_SHIFT + i * ADC_CONFIG_INTEN_CHAN_BIT_SIZE)
             |  config->adc_ch[i] << (ADC16_CONFIG_CHAN0_SHIFT + i * ADC_CONFIG_INTEN_CHAN_BIT_SIZE);
    }
