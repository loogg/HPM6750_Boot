...
void board_reset_camera(bool active)
{
    if (active) {
        /* reset, active low */
        gpio_write_pin(BOARD_LED_GPIO_CTRL, GPIO_DI_GPIOX, 8, 0);
    } else {
        /* clear reset */
       gpio_write_pin(BOARD_LED_GPIO_CTRL, GPIO_DI_GPIOX, 8, 1);
    }
}
...
void board_power_down_camera(bool active)
{
    if (active) {
        ...
        gpio_write_pin(BOARD_LED_GPIO_CTRL, GPIO_DI_GPIOX, 9, 1);
    } else {
        /* power up */
        gpio_write_pin(BOARD_LED_GPIO_CTRL, GPIO_DI_GPIOX, 9, 0);
    }
}
#endif
...
#ifdef CAMREA_RESET_PWDN_CONFIGURABLE
    /* RST */
    gpio_enable_pin_output(BOARD_LED_GPIO_CTRL, GPIO_DI_GPIOX, 8);
    /* PWNDN */
    gpio_enable_pin_output(BOARD_LED_GPIO_CTRL, GPIO_DI_GPIOX, 9);
#endif
...
void board_init_usb_pins(void)
{
    ...
    gpio_disable_pin_output(BOARD_USB1_ID_PORT, BOARD_USB1_ID_GPIO_INDEX, BOARD_USB1_ID_GPIO_PIN);
    ...
    gpio_disable_pin_output(BOARD_USB1_OC_PORT, BOARD_USB1_OC_GPIO_INDEX, BOARD_USB1_OC_GPIO_PIN);
}
...
hpm_stat_t board_init_enet_pins(ENET_Type *ptr)
{
    ...
    if (ptr == HPM_ENET0) {
        gpio_enable_pin_output_with_initial(BOARD_ENET_RGMII_RST_GPIO, BOARD_ENET_RGMII_RST_GPIO_INDEX, BOARD_ENET_RGMII_RST_GPIO_PIN, 0);
        board_delay_ms(1);
