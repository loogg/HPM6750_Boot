...
uart_rx_flexiable_data_context_t demo_config = {
    ...
    .uart_info = {
                .ptr = TEST_UART,
                .baudrate = 115200U,
                .buff_addr = (uint32_t)uart_rx_buff,
    },
    .dma_info = {
                .ptr = TEST_DMA,
                .ch = TEST_DMA_CHANNEL,
                .dst_addr = (uint32_t)dma_buf,
                .buff_size = BUFF_SIZE,
    },
    .dmamux_info = {
                .ptr = TEST_DMAMUX,
...
void uart_dma_rx_idle_callback(uart_rx_flexiable_data_context_t *demo_config)
{
    ...
    demo_config->uart_receive_data_size = dma_info.ptr->CHCTRL[dma_info.ch].DSTADDR
                                          - core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)dma_info.dst_addr);
...
void setup_uart_flexible_data_reception(uart_rx_flexiable_data_context_t *config)
{
    ...
    configure_uart_dma_transfer(&config->uart_info, &config->dma_info, &config->dmamux_info);
