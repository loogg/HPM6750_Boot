...
void process_preempt_data(uint32_t *buff, uint32_t len)
{
    adc16_preempt_dma_data_t dma_data[len];
...
void init_common_config(adc16_conversion_mode_t conv_mode)
{
    ...
    if (cfg.conv_mode == adc16_conv_mode_sequence ||
        cfg.conv_mode == adc16_conv_mode_preempt) {
        cfg.adc_ahb_en = true;
    }
...
void init_period_config(void)
{
    ...
    /* prd_cfg.clk_src_in_hz = */

    adc16_set_period_config(BOARD_APP_ADC16_BASE, &prd_cfg);
}
...
void init_sequence_config(void)
{
    ...
    adc16_set_sequence_config(BOARD_APP_ADC16_BASE, &seq_cfg);
...
void init_preemption_config(void)
{
    ...
    adc16_init_preempt_dma(BOARD_APP_ADC16_BASE, core_local_mem_to_sys_address(BOARD_APP_CORE, (uint32_t)preempt_buffer));
}
...
void preempt_handler(void)
{
    ...
    /* Process data */
    process_preempt_data(preempt_buffer, BOARD_APP_ADC_SINGLE_CONV_CNT);
...
int main(void)
{
    ...
    while (1) {
        if (conv_mode == adc16_conv_mode_oneshot) {
            ...
            seq_handler();
        } else if (conv_mode == adc16_conv_mode_preempt) {
            preempt_handler();
        } else {
